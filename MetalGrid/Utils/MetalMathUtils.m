...
static vector_float3 MTL_SIMD_OVERLOAD vector_make(float x, float y, float z) {
    return (vector_float3){ x, y, z };
}

vector_float3 MTL_SIMD_OVERLOAD vector_lerp(vector_float3 v0, vector_float3 v1, float t) {
    return ((1 - t) * v0) + (t * v1);
}

vector_float4 MTL_SIMD_OVERLOAD vector_lerp(vector_float4 v0, vector_float4 v1, float t) {
    return ((1 - t) * v0) + (t * v1);
}

static matrix_float3x3 MTL_SIMD_OVERLOAD matrix_make(float m00, float m10, float m20,
...
static matrix_float3x3 MTL_SIMD_OVERLOAD matrix_make(vector_float3 col0, vector_float3 col1, vector_float3 col2) {
    return (matrix_float3x3){ col0, col1, col2 };
}

matrix_float3x3 MTL_SIMD_OVERLOAD matrix3x3_from_quaternion(quaternion_float q) {
    float xx = q.x * q.x;
    float xy = q.x * q.y;
    float xz = q.x * q.z;
    float xw = q.x * q.w;
...
matrix_float3x3 MTL_SIMD_OVERLOAD matrix3x3_rotation(float radians, vector_float3 axis) {
...
matrix_float3x3 MTL_SIMD_OVERLOAD matrix3x3_rotation(float radians, float x, float y, float z) {
    return matrix3x3_rotation(radians, vector_make(x, y, z));
}

matrix_float3x3 MTL_SIMD_OVERLOAD matrix3x3_scale(float sx, float sy, float sz) {
    return matrix_make(sx, 0, 0, 0, sy, 0, 0, 0, sz);
}

matrix_float3x3 MTL_SIMD_OVERLOAD matrix3x3_scale(vector_float3 s) {
    return matrix_make(s.x, 0, 0, 0, s.y, 0, 0, 0, s.z);
}

matrix_float3x3 MTL_SIMD_OVERLOAD matrix_inverse_transpose(matrix_float3x3 m) {
    return matrix_invert(matrix_transpose(m));
}

static matrix_float4x4 MTL_SIMD_OVERLOAD matrix_make(float m00, float m10, float m20, float m30,
...
matrix_float4x4 MTL_SIMD_OVERLOAD matrix4x4_from_quaternion(quaternion_float q) {
    float xx = q.x * q.x;
    float xy = q.x * q.y;
    float xz = q.x * q.z;
    float xw = q.x * q.w;
...
matrix_float4x4 MTL_SIMD_OVERLOAD matrix4x4_rotation(float radians, vector_float3 axis) {
    axis = vector_normalize(axis);
    float ct = cosf(radians);
    float st = sinf(radians);
    float ci = 1 - ct;
...
matrix_float4x4 MTL_SIMD_OVERLOAD matrix4x4_rotation(float radians, float x, float y, float z) {
    return matrix4x4_rotation(radians, vector_make(x, y, z));
}

matrix_float4x4 MTL_SIMD_OVERLOAD matrix4x4_scale(float sx, float sy, float sz) {
...
matrix_float4x4 MTL_SIMD_OVERLOAD matrix4x4_scale(vector_float3 s) {
...
matrix_float4x4 MTL_SIMD_OVERLOAD matrix_inverse_transpose(matrix_float4x4 m) {
    return matrix_invert(matrix_transpose(m));
}

quaternion_float MTL_SIMD_OVERLOAD quaternion(float x, float y, float z, float w) {
    return (quaternion_float){ x, y, z, w };
}

quaternion_float MTL_SIMD_OVERLOAD quaternion(vector_float3 v, float w) {
    return (quaternion_float){ v.x, v.y, v.z, w };
}

quaternion_float MTL_SIMD_OVERLOAD quaternion_from_axis_angle(vector_float3 axis, float radians);
quaternion_float MTL_SIMD_OVERLOAD quaternion_from_axis_angle(vector_float3 axis, float radians) {
    float t = radians * 0.5;
    return quaternion(axis.x * sinf(t), axis.y * sinf(t), axis.z * sinf(t), cosf(t));
}

quaternion_float MTL_SIMD_OVERLOAD quaternion(matrix_float3x3 m) {
...
quaternion_float MTL_SIMD_OVERLOAD quaternion(matrix_float4x4 m) {
...
vector_float3 MTL_SIMD_OVERLOAD quaternion_rotate_vector(quaternion_float q, vector_float3 v) {
